...
public class HybridQuickSort extends QuickSort{
    //segment size at which quickSort switches over to insertionSort
    private static final int STOP_SORT_HERE = 30;
    ...
    /**
     * private recursive method; sorts array from least to greatest via hybrid
     * quick sort function
    ...
    private void sortListUtil(int[] array, int begin, int end){
        /**
         * base case. if segment is smaller than STOP_SORT_HERE, switch to insertionSort
         * and return when sorted
         */
        if(end - begin < STOP_SORT_HERE){
    ...
    /**
     * Sorts in place a section of an array using insertion sort
     * @param array the array
     * @param begin starting coordinate of section you wish to sort
     * @param end ending coordinate of section you wish to sort
     */
    private void insertionSort(int[] array, int begin, int end){
        //for each location in section
        for(int i = begin; i <= end; i++){
            for(int j = begin; j < i; j++){
                //compare it with contents of every location before it in the section
                if(array[i] < array[j]){
                    //if current location contents < previous location contents, switch
                    int hold = array[i];
                    array[i] = array[j];
                    array[j] = hold;
                }
