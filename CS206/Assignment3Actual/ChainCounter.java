...
public class ChainCounter{
    ...
    private int chainSize = 70000;//size of ProbeHTInc array

    private int uniqueWords = 0; //number of unique words in file
    
    private SepChainHT<String, Integer> chain = new SepChainHT<>(chainSize);
     ...
     */
    public ChainCounter(String filename){
     ...
     */
    public SepChainHT<String, Integer> getChain(){
        return chain;
    } 
        ...
        try{
            ...
            while(read.isReady() == true){
                ...
                if(!chain.containsKey(word)){
                    ...
                    //with wordCount of 1
                    chain.put(word, wordCount);
                }else if(chain.containsKey(word)){
                    //gets current number of word occurrences and adds 1
                    wordCount = chain.get(word) + 1;
    ...
    public void bestOfThree(){
        ChainCounter cc = new ChainCounter(filename);
        cc.countWords();
        cc.sort();

        int[] counts = cc.getTopThreeCounts();
        String[] words = cc.getTopThreeWords();

        System.out.println("File: " + filename);
        System.out.println("Number of unique words: " + cc.getUniqueWordCount());
     ...
     */
    private void sort(){
        ArrayList<String> keys = chain.returnKeyArrayList(); //unique words in text file
        ArrayList<Integer> values = chain.returnValueArrayList(); //unique words' associated word counts
